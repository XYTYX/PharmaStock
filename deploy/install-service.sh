...
# install-service.sh
# Script to install PharmaStock as a systemd service with pharmacy routing
...
copy_application_files() {
    ...
    chmod +x "$APP_DIR/setup-pharmacy-routing.sh"
...
setup_pharmacy_routing() {
    log "Setting up pharmacy routing..."
    
    if [ -f "$APP_DIR/setup-pharmacy-routing.sh" ]; then
        chmod +x "$APP_DIR/setup-pharmacy-routing.sh"
        "$APP_DIR/setup-pharmacy-routing.sh" setup
        log "Pharmacy routing setup completed"
    else
        log "WARNING: setup-pharmacy-routing.sh not found"
    fi
}
...
show_service_info() {
    ...
    log ""
    log "Application URL: http://new-sight.local/pharmacy/"
    log "Backend API: http://new-sight.local/pharmacy/api/"
    log ""
    log "Useful commands:"
    ...
    log "  Setup routing:   $APP_DIR/setup-pharmacy-routing.sh"
    log ""
...
show_usage() {
    echo "Usage: $0 {install|uninstall|status|routing}"
